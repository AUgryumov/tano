...
fn usual_neuron() {
    ...
    assert_eq!(neuron.run(&vec![1.], super::super::utils::no_activation), 1.);
    assert_eq!(neuron.run(&vec![0.], super::super::utils::no_activation), 0.);

    neuron.get_mut_weights()[0] = 0.;
    assert_eq!(neuron.run(&vec![1.], super::super::utils::no_activation), 0.);
    assert_eq!(neuron.run(&vec![0.], super::super::utils::no_activation), 0.);

    neuron.get_mut_weights()[0] = 0.5;
    assert_eq!(neuron.run(&vec![1.], super::super::utils::no_activation), 0.5);
    assert_eq!(neuron.run(&vec![0.], super::super::utils::no_activation), 0.);
}
